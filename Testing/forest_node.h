#ifndef TESTING_FOREST_NODE_H
#define TESTING_FOREST_NODE_H
...
int ForestNode::setUnion(int a, int b, vector<ForestNode> &forest) {
    ...
    if (forest[b].height > forest[a].height ||
        (forest[b].height == forest[a].height && !forest[a].is_edge && forest[b].is_edge)) {  //treats the case when one inside and one outside and I want to keep consistent id
